...
var LEAVES_INDEX = 6;
var NODES_INDEX = 7;
var babyBlue = [0.537, 0.812, 0.941, 1];
var smoothBrown = [1, 0.7, 0.286, 1];
/**
...
function MySceneGraph(filename, scene) {
	...
	this.selectableNodes = ['not defined'];
...
MySceneGraph.prototype.parseAnimations = function (animationsNode) {
    ...
    for (var i = 0; i < children.length; i++) {
        ...
        if (animationType == null)
            return "no Type defined for animation (conflict: ID = " + animationId + ")";

        if (animationType != "linear" && animationType != "circular" && animationType != "bezier" && animationType != "combo")
            return "Type can only be defined as linear, circular, bezier or combo (conflict: Type = " + animationType + ")";

        if (animationSpeed == null && animationType != "combo")
            return "no speed defined for animation";
        else if (isNaN(animationSpeed)) {
            return "Animation speed must be a floating point number (conflict: Speed = " + animationSpeed + ")";
        }

        if (this.animations[animationId] != null)
            return "ID must be unique for each animation (conflict: ID = " + animationId + ")";

        //Circular animations parse

        if (animationType == "circular") {
            var centerx = this.reader.getFloat(children[i], 'centerx');

            if (centerx == null)
                return "unable to parse centerx component of circular animation for animation with ID = " + animationId;
            else if (isNaN(centerx))
                return "Circular animation centerx must be a floating point number (conflict: centerx = " + centerx + ")";

            var centery = this.reader.getFloat(children[i], 'centery');

            if (centery == null)
                return "unable to parse centery component of circular animation for animation with ID = " + animationId;
            else if (isNaN(centery))
                return "Circular animation centery must be a floating point number (conflict: centery = " + centery + ")";

            var centerz = this.reader.getFloat(children[i], 'centerz');

            if (centerz == null)
                return "unable to parse centerz component of circular animation for animation with ID = " + animationId;
            else if (isNaN(centerz))
                return "Circular animation centerz must be a floating point number (conflict: centerz = " + centerz + ")";

            var radius = this.reader.getFloat(children[i], 'radius');

            if (radius == null)
                return "unable to parse radius component of circular animation for animation with ID = " + animationId;
            else if (isNaN(radius))
                return "Circular animation radius must be a floating point number (conflict: radius = " + radius + ")";

            var startang = this.reader.getFloat(children[i], 'startang');

            if (startang == null)
                return "unable to parse startang component of circular animation for animation with ID = " + animationId;
            else if (isNaN(startang))
                return "Circular animation startang must be a floating point number (conflict: startang = " + startang + ")";

            var rotang = this.reader.getFloat(children[i], 'rotang');


            if (rotang == null)
                return "unable to parse rotang component of circular animation for animation with ID = " + animationId;
            else if (isNaN(rotang))
                return "Circular animation rotang must be a floating point number (conflict: rotang = " + rotang + ")";

            this.animations[animationId] = new MyCircularAnimation(this.scene, [[centerx, centery, centerz], radius, startang, rotang, animationSpeed]);

        } else if (animationType == "linear" || animationType == "bezier") {

            //Linear and Bezier animations parse
            ...
            for (var j = 0; j < animationSpecs.length; j++) {
                ...
                //control points
                var controlpoint = animationSpecs[j];
                if (controlpoint.nodeName != "controlpoint")
                    return "only controlpoints can be defined for linear / bezier animations (conflict: animationID = " + animationId + ")";

                let cpCoords = [];
                // xx
                var xx = this.reader.getFloat(controlpoint, 'xx');
                if (xx == null)
                    return "unable to parse xx component for circular animation with ID = " + animationId;
                else if (isNaN(xx))
                    return "'xx' is a non numeric value on the ANIMATIONS block (conflict: animationID = " + animationId + ")";
                cpCoords.push(xx);
                // yy
                var yy = this.reader.getFloat(controlpoint, 'yy');
                if (yy == null)
                    return "unable to parse yy component for circular animation with ID = " + animationId;
                else if (isNaN(yy))
                    return "'yy' is a non numeric value on the ANIMATIONS block (conflict: animationID = " + animationId + ")";
                cpCoords.push(yy);
                // zz
                var zz = this.reader.getFloat(controlpoint, 'zz');
                if (zz == null)
                    return "unable to parse zz component for circular animation with ID = " + animationId;
                else if (isNaN(zz))
                    return "'zz' is a non numeric value on the ANIMATIONS block (conflict: animationID = " + animationId + ")";
                cpCoords.push(zz);
            ...
            if (animationType == "linear") {
                this.animations[animationId] = new MyLinearAnimation(this.scene, [cps, animationSpeed]);
            } else if (animationType == "bezier") {
                this.animations[animationId] = new MyBezierAnimation(this.scene, [cps, animationSpeed]);
            }
        } else if (animationType == "combo") {
            var animationRefs = children[i].children;
            var currentComboAnimatios = [];
            for (var j = 0; j < animationRefs.length; j++) {
                var id = this.reader.getString(animationRefs[j], 'id');
                if (id == "") {
                    return "Id of combo animation must be defined (conflict: animationID = " + animationId + ")";
                }
                var animationReference = this.animations[id];
                if (animationReference == null) {
                    return "Id " + id + " has no animation referenced to it (conflict: animationID = " + animationId + ")";
                } else {
                    currentComboAnimatios.push(animationReference);
                }
            }
            this.animations[animationId] = currentComboAnimatios;
        }
    }
...
MySceneGraph.prototype.parseNodes = function (nodesNode) {
    ...
    for (var i = 0; i < children.length; i++) {
        ...
        if ((nodeName = children[i].nodeName) == "ROOT") {
            ...
            if(children[i].hasAttribute('scene')){
                var sceneDisplay = this.reader.getString(children[i], 'scene');
                if(sceneDisplay != null)
                    nodeScene = sceneDisplay;
            }

            // Creates node.
            this.nodes[nodeID] = new MyGraphNode(this, nodeID);
            this.nodes[nodeID].displayScene = nodeScene;
			//Analyze node selectabilty
            if (children[i].hasAttribute('selectable')) {
                var selectable = this.reader.getString(children[i], 'selectable');
                if (selectable != "false" && selectable != "true") {
                    return "selectable argument must be either 'true' or 'false' (conflict: ID = " + nodeID + ")";
            ...
            if (animationsIndex != -1) {
                //This node has animations
                var animationsRefs = nodeSpecs[animationsIndex].children;
                for (let j = 0; j < animationsRefs.length; j++) {
                    var animationID = this.reader.getString(animationsRefs[j], 'id');
                    if (animationID == null)
                        return "Animation ID cannot be null! (node ID = " + nodeID + ")";

                    var currentAnimation = this.animations[animationID];
                    if (currentAnimation.constructor === Array) {
                        //Combo Animation
                        for (let k = 0; k < currentAnimation.length; k++) {
                            if (currentAnimation[k] == undefined) {
                                return "Animation ID must be refered on the ANIMATIONS section (node ID = " + nodeID + ")";
                            }
                            else {
                                this.nodes[nodeID].animations.push(currentAnimation[k]);
                            }
                        }
                    }
                    else if (currentAnimation == undefined) {
                        return "Animation ID must be refered on the ANIMATIONS section (node ID = " + nodeID + ")";
                    }
                    else {
                        this.nodes[nodeID].animations.push(currentAnimation);
                    }
                }
            }
...
MySceneGraph.prototype.displayScene = function () {
    ...
    if(this.selectedScene == "1") {
        this.scene.gl.clearColor(babyBlue[0], babyBlue[1], babyBlue[2], babyBlue[3]);
    }
    else if(this.selectedScene == "2")
        this.scene.gl.clearColor(smoothBrown[0], smoothBrown[1], smoothBrown[2], smoothBrown[3]);
    // entry point for graph rendering
    this.dfs(this.nodes[this.idRoot], this.nodes[this.idRoot].materialID, this.nodes[this.idRoot].textureID);
}

MySceneGraph.prototype.dfs = function (currentNode, materialID, textureID, showNode="all") {

    var currentMaterialID = materialID;
    var currentTextureID = textureID;

    if (currentNode == null || currentNode.nodeID == null)
        return 1;
	
    if (currentNode.materialID != "null")
        currentMaterialID = currentNode.materialID;

    if (currentNode.textureID != "null")
        currentTextureID = currentNode.textureID;
    ...
    if (currentTextureID != "null") {
        //set ampliation factors for all leaves
        for (let j = 0; j < currentNode.leaves.length; j++) {
            var amplifFactorS = this.textures[currentTextureID][1];
            var amplifFactorT = this.textures[currentTextureID][2];
            currentNode.leaves[j].setAmplifFactor(amplifFactorS, amplifFactorT);
        }
        this.textures[currentTextureID][0].bind();
    }

	if(currentNode.nodeID == this.scene.currSelectableID)
		this.scene.setActiveShader(this.scene.shaders[this.scene.currShaderIndex]);
	
    this.scene.multMatrix(currentNode.getMatrix());


    for (let i = 0; i < currentNode.children.length; i++) {
        this.scene.pushMatrix();
        if(showNode == "all")
        this.dfs(this.nodes[currentNode.children[i]], currentMaterialID, currentTextureID, currentNode.displayScene);
        else
        this.dfs(this.nodes[currentNode.children[i]], currentMaterialID, currentTextureID, showNode);
        this.scene.popMatrix();
    }

    for (let j = 0; j < currentNode.leaves.length; j++) {
        if(this.scene.wireframe){
            currentNode.leaves[j].togglePrimitiveType(this.scene.gl.LINES);
        } else {
            currentNode.leaves[j].togglePrimitiveType(this.scene.gl.TRIANGLES);
        }

        if(showNode == this.selectedScene || showNode == "all" ){
            this.scene.pushMatrix();
            currentNode.leaves[j].displayObject.display();
            this.scene.popMatrix();
        }
    }
